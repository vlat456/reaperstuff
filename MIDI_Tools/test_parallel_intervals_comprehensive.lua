-- @noindex
-- @description Comprehensive test suite for parallel interval detection
...
-- Helper function to create mock take
local function create_mock_take()
    return {
        is_mock = true,
        notes = {}
    }
...
local function test_4note_parallel_fifths()
    ...
    -- Should detect multiple parallel fifths in 4-note texture
    ...
    local found_g_d = false
    local found_e_b = false
    
    for _, error in ipairs(errors) do
        if error.pitch_A_curr == 60 and error.pitch_B_curr == 67 then
            found_c_g = true
        end
        if error.pitch_A_curr == 67 and error.pitch_B_curr == 74 then
            found_g_d = true
        end
        if error.pitch_A_curr == 64 and error.pitch_B_curr == 71 then
            found_e_b = true
        end
    end
...
-- Test 6: Parallel fourths with different chord sizes
local function test_parallel_fourths()
    reaper.ShowConsoleMsg("Testing parallel fourths with different chord sizes\n")
    
    -- Test with 3-note chords
    local notes_3note = {
        create_mock_note(60, 0),   -- C
        create_mock_note(64, 0),   -- E
        create_mock_note(65, 0),   -- F (perfect fourth above C)
        create_mock_note(62, 480), -- D
        create_mock_note(66, 480), -- F#
        create_mock_note(69, 480)  -- A (perfect fourth above E, but not above D)
    }
    
    local chords_3note = PARALLEL_DETECTOR.group_notes_into_chords(notes_3note)
    local errors_fourths = PARALLEL_DETECTOR.analyze_parallel_intervals(chords_3note, PARALLEL_DETECTOR.is_perfect_fourth)
    
    TestFramework.assert_equal(#errors_fourths, 1, "Should detect 1 parallel fourth in 3-note chords")
    
    -- Test with 4-note chords
    local notes_4note = {
        create_mock_note(60, 0),   -- C
        create_mock_note(64, 0),   -- E
        create_mock_note(65, 0),   -- F
        create_mock_note(69, 0),   -- A
        create_mock_note(62, 480), -- D
        create_mock_note(66, 480), -- F#
        create_mock_note(69, 480), -- A
        create_mock_note(73, 480)  -- C#
    ...
    local chords_4note = PARALLEL_DETECTOR.group_notes_into_chords(notes_4note)
    local errors_fourths_4note = PARALLEL_DETECTOR.analyze_parallel_intervals(chords_4note, PARALLEL_DETECTOR.is_perfect_fourth)
    
    TestFramework.assert(#errors_fourths_4note >= 1, "Should detect at least 1 parallel fourth in 4-note chords")
end

-- Test 7: Edge cases - no parallel intervals
local function test_no_parallel_intervals()
    ...
    -- Test for parallel fourths
    local errors_fourths = PARALLEL_DETECTOR.analyze_parallel_intervals(chords, PARALLEL_DETECTOR.is_perfect_fourth)
    TestFramework.assert_equal(#errors_fourths, 0, "Should not detect parallel fourths in contrary motion")
end

-- Test 8: Static voices (should not be detected as parallel)
...
-- Test 9: Complex multi-chord progression
local function test_complex_progression()
    reaper.ShowConsoleMsg("Testing complex multi-chord progression\n")
    
    -- Create a 4-chord progression with various parallel intervals
    local notes = {
        -- Chord 1: C major
        create_mock_note(60, 0),   -- C
        create_mock_note(64, 0),   -- E
        ...
        -- Chord 2: G major (parallel fifth from C-E to G-B)
        create_mock_note(67, 480), -- G
        create_mock_note(71, 480), -- B
        create_mock_note(74, 480), -- D
        
        -- Chord 3: D major (parallel fifth from G-B to D-F#)
        create_mock_note(62, 960), -- D
        create_mock_note(66, 960), -- F#
        create_mock_note(69, 960), -- A
        
        -- Chord 4: A major (parallel fifth from D-F# to A-C#)
        create_mock_note(69, 1440), -- A
        create_mock_note(73, 1440), -- C#
        create_mock_note(76, 1440)  -- E
    }
    
    local chords = PARALLEL_DETECTOR.group_notes_into_chords(notes)
    
    TestFramework.assert_equal(#chords, 4, "Should create 4 chords")
    
    -- Test for parallel fifths
    local errors = PARALLEL_DETECTOR.analyze_parallel_intervals(chords, PARALLEL_DETECTOR.is_perfect_fifth)
    
    -- Should detect multiple parallel fifths across the progression
    TestFramework.assert(#errors >= 2, "Should detect multiple parallel fifths in progression")
end

-- Test 10: Interval detection accuracy
...
local function run_all_tests()
    reaper.ShowConsoleMsg("Starting Comprehensive Parallel Interval Detection Tests\n")
    reaper.ShowConsoleMsg("=====================================================\n")
    ...
    TestFramework.run_test("Parallel fourths with different chord sizes", test_parallel_fourths)
    ...
    TestFramework.run_test("Complex multi-chord progression", test_complex_progression)
