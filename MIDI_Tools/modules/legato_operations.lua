...
function M.apply_legato_with_extension(current_take, selected_notes, extension_percentage)
    ...
    for i, note in ipairs(selected_notes) do
        ...
        if next_note then
            local new_end_ppq = next_note.startppqpos
            ...
            local next_note_length_ms = (end_time - start_time) * 1000
            local extension_ms = math.max(next_note_length_ms * extension_percentage, 1)
